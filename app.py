...
# ====================== FILTERS ======================
# Reformat period labels: weeks as 2026-W-01, months as Jan-26
...
# --- Cascading filters: each one narrows the options of the next ---
...
if mode == "Weeks":
    all_weeks = sorted(base['week'].unique())            # only weeks in the selected year(s)
    selected_weeks = st.sidebar.multiselect("Select Weeks", all_weeks, default=all_weeks[-8:])
    if selected_weeks:
        base = base[base['week'].isin(selected_weeks)]
else:
    # only months in the selected year(s), in chronological order
...
# Regions: only those present in the selected market(s), e.g. Ethiopia -> Ethiopian regions
...
weekly_nps = weekly_nps.sort_values('week')   # 2026-W-01 format sorts correctly as text
