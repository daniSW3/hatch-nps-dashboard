...
if mode == "Weeks":
    all_weeks = sorted(df['week'].unique())          # 2026-W-01 sorts correctly as text
    ...
    # Jan-26 doesn't sort alphabetically — order chronologically by the underlying date
    all_months = df.sort_values('created_date')['month'].unique().tolist()
...
# Region filter — options follow the selected markets
